# Refactoring — Full Reference Tier

**Attribution:** This file is the `full` tier from the `refactoring` rule set in the
[agent-rules-books](https://github.com/ciembor/agent-rules-books) project by Maciej
Ciemborowicz (MIT licensed), itself a distillation of Martin Fowler's *Refactoring*. It is
reproduced here verbatim as reference material for this skill — use it for deep audits, a
one-off deep-dive session, or when the compressed skill body in `SKILL.md` doesn't cover a
situation in enough detail. It is not the book itself and is not a substitute for it.
