# Working Effectively with Legacy Code — Full Reference Tier

**Attribution:** This file is the `full` tier from the `working-effectively-with-legacy-code`
...
Effectively with Legacy Code*. It is reproduced here verbatim as reference material for this
skill — use it for deep audits, a one-off deep-dive session, or when the compressed skill
body in `SKILL.md` doesn't cover a situation in enough detail (in particular, the full seam
catalog and dependency-breaking technique index below go well beyond what's practical to keep
in the always-on skill body). It is not the book itself and is not a substitute for it.
