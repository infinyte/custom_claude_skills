# Refactoring — Nano Quick Reference

**Attribution:** This file is the `nano` tier from the `refactoring` rule set in the
[agent-rules-books](https://github.com/ciembor/agent-rules-books) project by Maciej
Ciemborowicz (MIT licensed), itself a distillation of Martin Fowler's *Refactoring*. It's an
extremely compact fallback — use it only when even the SKILL.md body is too large for the
