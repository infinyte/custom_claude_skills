# Release It! — Nano Quick Reference

**Attribution:** This file is the `nano` tier from the `release-it` rule set in the
[agent-rules-books](https://github.com/ciembor/agent-rules-books) project by Maciej
Ciemborowicz (MIT licensed), itself a distillation of Michael T. Nygard's *Release It!*. It's
an extremely compact fallback — use it only when even the SKILL.md body is too large for the
available context (e.g., stacking several skills' guidance at once, or a tool with a very
small always-on instruction budget). Prefer the main SKILL.md body over this file whenever
context allows.
